export function updateList<DataType extends { [key: string]: any }, Key = any>({
    ...
    actions.forEach(action => {
        ...
        if (objectLevels?.length === 1 && key && action === "update") {
            newRef.forEach(item => {
                if (
                    item[objectLevels[0]][conditionKey] === id?.replace("/", "")
                )
            ...
            newRef.forEach(item => {
                if (item[conditionKey] === id?.replace("/", ""))
                    item[key] = valueOfKey;
                newList.push(item);
            });
            newRef = JSON.parse(JSON.stringify(newList));
            ...
            if (!isOn) {
                ...
                if (_newObject[conditionKey] === id?.replace("/", ""))
                    _newObject[key] = valueOfKey;
            ...
            newList = newList.filter(
                item => item[conditionKey] !== id?.replace("/", ""),
            );
            newRef = JSON.parse(JSON.stringify(newList));
        }
    });
